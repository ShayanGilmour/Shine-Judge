...
wget http://MainIP/problems/prob20/checker.cpp
g++ checker.cpp
...
echo -ne "Test 0: " >> /var/www/html/result

./a.out < code.py
if [ ! -f qualified ]; then
        echo -ne "Wrong\n\n" >> /var/www/html/result
        echo -ne "Using int is forbidden for this problem." >> /var/www/html/result
...
do
	...
	OUTPUT="$(timeout 0.1 python3 code.py < input$i > output$i)"
	status=$?
#	echo $status &> what
	if ((status==124)); then
	{
